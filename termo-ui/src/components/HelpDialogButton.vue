...
<template>
  <Dialog>
    ...
    <DialogContent class="dark text-foreground bg-secondary overflow-y-auto max-h-[80vh]">
      <p>
        Find the correct word in 6 tries. After each try, the tiles show how close you are from
        getting it right.
      </p>

      <div class="flex gap-1">
        <BoardTile
          v-for="tile in rowWithCorrectLetter.tiles"
          :tile="tile"
          :row="rowWithCorrectLetter"
          class="w-12 h-12 text-3xl"
        />
      </div>
      <p>
        The letter
        <BoardTile :tile="correctTile" :row="rowWithCorrectLetter" class="w-8 h-8 mx-0.5 text-xl" />
        is part of the word and it is in the correct position.
      </p>

      <div class="flex gap-1">
        <BoardTile
          v-for="tile in rowWithLetterInWrongPosition.tiles"
          :tile="tile"
          :row="rowWithLetterInWrongPosition"
          class="w-12 h-12 text-3xl"
        />
      </div>
      <p>
        ...
        <BoardTile
          :tile="presentTile"
          :row="rowWithLetterInWrongPosition"
          class="w-8 h-8 mx-0.5 text-xl"
        />
        is part of the word but in another position.
      </p>

      <div class="flex gap-1">
        <BoardTile
          v-for="tile in rowWithAbsentWord.tiles"
          :tile="tile"
          :row="rowWithAbsentWord"
          class="w-12 h-12 text-3xl"
        />
      </div>
      <p>
        The letter
        <BoardTile :tile="absentTile" :row="rowWithAbsentWord" class="w-8 h-8 mx-0.5 text-xl" /> is
